---
title: "이탈리아 유학, 박사과정 입학기 - 비자"
tags: [phd, visa, studying_abroad]
categories: phd
sidebar:
  nav: "docs"
...
{% capture notice-2 %}
시작하기 앞서서 어느나라든 해외유학을 시작하려는 여러분들에게,

한국과 달리 **행정처리 속도가 상당이 느린편**이므로 **<u>충분한 기간을 두고 비자 및 필요한 일을 처리하길 바랍니다.</u>**

공휴일 및 특별사정으로 인해 행정처리 도중 **돌발상황이 발생**하므로 여유를 두고 신청하세요.
{% endcapture %}
...
7. 체류하는 동안 머물 숙소 서류 (아래에서 1가지 선택해 제출).
    ...
    *임시 숙소의 경우 이탈리아 정부에 등록, 세금을 내는 합법적인 호텔 숙소만 인정됩니다.
    > 숙소 계약시, 계약서+집주인 여권사본+집주인 임대등록서(관할 Agenzia delle entrate에 문의해 발급)제출.
...
비자 신청서는 따로 설명할 필요 없이 친절하게 어떤내용을 기입하면 될지 안내되어 있다.
차근차근 천천히, 오탈자가 발생하지 않도록 주의하며 작성해나가자.
...
국문기본증명서는 대한민국 정부24에서 검색하거나, 또는 본 업무가 대법원관할이므로 전자가족관계등록시스템에 접속 [https://efamily.scourt.go.kr](https://efamily.scourt.go.kr)하여 인쇄하도록 하자. 2024년 기준 PDF저장 가능.
...
## 체류기간 머물 숙소
비록 나는 학교에서 제공하는 기숙사에 머물 예정이긴 했으나, 비자를 발급받기 위해 방문하기 전 시점 아무것도 확정된게 없었다.
(지금와서 돌아보건데 기숙사측에 연락이라도 해볼 걸 그랬다.)
따라서 나는 임시로 한 달간 머물 숙소를 결제했다.
결제금액이 상당한 편이라 되도록 많은 금액이 바로 빠져나가지 않도록 신용카드를 사용하는 것도 방법.
물론 인터뷰 및 비자 발급 이후 숙소는 모두 취소하고 기숙사에 머물기 전까지 숙소는 따로 잡아서 해결.
...
{% endcapture %}
<div class="notice--caution">{{ notice-2 | markdownify }}</div>

대기시간과 업무 처리시간을 포함하여 20-30분 정도 소요되니 여유를 두고 신청하자.

## Universitaly
Universitaly는 이탈리아 정부에서 주관하여 인터넷으로 확인을 거치는 시스템이다.
아마도 대부분의 이탈리아 현지 대학교들이 해당 시스템을 이용하여 외국학생을 받는 것으로 파악되며,
이 시스템을 이용하여 학교, 정부, 현지 파견 이탈리아 대사관간 연락이 취해지는 것으로 보인다.
Universitaly를 통하여 등록을 진행하면 마지막에 등록확인증을 발급받을 수 있는데, 이 문서를 인쇄하여 인터뷰 전 대사관에 방문하면 된다. 
나의 경우 다음의 순서로 진행되었다.

{% capture notice-2 %}
1. University of Trento에서 합격통지 수령
2. Universitaly를 통한 등록
3. 지원 학교측에서 지원자의 Universitaly를 통한 등록 확인
3. Universitaly 시스템에서 지원자 국가의 이탈리아 대사관으로 정보 전달
4. 대사관 인터뷰
5. 대사관으로부터 Universitaly 시스템에서 승인
6. 등록 학교의 최종 확인
{% endcapture %}
<div class="notice--info">{{ notice-2 | markdownify }}</div>
